...
class BattleWindow(TopLevelWindow):
    ...
    def draw(self) -> TopLevelWindow:
        ...
        def on_catch_start(ball: Ball):
            ...
            # Calculate and store the exact x-position before creating the new label
            opponent_x = current_opponent_image.winfo_x()
            ...
            poke_ball_image.place(x=opponent_x, y=current_opponent_image.winfo_y(), anchor=tk.NE)

            # Store the original position immediately after placement
            poke_ball_original_x = opponent_x
        ...
        def on_catch_shake():
            ...
            def _shake(count=0, direction=2):
                ...
                if count >= 6:  # 3 full oscillations (shake)
        ...
        def create_button(text: str, row: int, col: int, bg: str, active_bg: str, callback: Callable[[], None]):
            # Create the button
            button = tk.Button(
                action_frame,
                text=text,
        ...
        def catch():
            ...
            self.battle.catch(Ball.POKE_BALL)
        ...
        # Create the buttons
        create_button("FIGHT", 0, 0, "#f96d5f", "#b44d43", fight)
        create_button("CATCH", 0, 1, "#ffd233", "#c9a72c", catch)
        create_button("POKEMON", 1, 0, "#61d18a", "#3c8757", pokemon)
        create_button("RUN", 1, 1, "#5f7ff9", "#354a97", run)
